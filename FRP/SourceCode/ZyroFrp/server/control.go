...
func (ctl *Control) RegisterProxy(pxyMsg *msg.NewProxy) (remoteAddr string, err error) {
	// 隧道鉴权：检查隧道是否在数据库中注册
	if ctl.loginMsg.UserKey != "" {

		// 查询用户所有已注册的隧道
		allowedTunnels, err := db.GetUserTunnels(ctl.loginMsg.UserKey)
		if err != nil {
			ctl.xl.Errorf("查询用户隧道列表失败: %v", err)
			return "", fmt.Errorf("隧道鉴权失败: %v", err)
		}

		// 查找匹配的隧道
		var matchedTunnel *db.TunnelInfo
		for _, tunnel := range allowedTunnels {
			if tunnel.TunnelName == pxyMsg.ProxyName {
		...
		if matchedTunnel == nil {
			ctl.xl.Errorf("隧道 '%s' 未在数据库中注册", pxyMsg.ProxyName)
			return "", fmt.Errorf("[001] 隧道不存在或未授权")
		}

		// 服务端标识验证
		if matchedTunnel.ServerName != ctl.serverCfg.ServerName {
			ctl.xl.Errorf("隧道 '%s' 不属于当前服务端 (期望: %s, 实际: %s)",
				pxyMsg.ProxyName, ctl.serverCfg.ServerName, matchedTunnel.ServerName)
			return "", fmt.Errorf("[002] 服务端标识不匹配")
		}

		// 隧道类型验证
		if matchedTunnel.TunnelType != pxyMsg.ProxyType {
			ctl.xl.Errorf("隧道 '%s' 类型不匹配 (期望: %s, 实际: %s)",
				pxyMsg.ProxyName, matchedTunnel.TunnelType, pxyMsg.ProxyType)
			return "", fmt.Errorf("[003] 隧道类型不匹配")
		}
		...
		case "tcp", "udp", "tcpmux":
			// 验证远程端口
			if matchedTunnel.RemotePort == nil {
				ctl.xl.Errorf("隧道 '%s' 缺少远程端口配置", pxyMsg.ProxyName)
				return "", fmt.Errorf("[004] 远程端口配置缺失")
			}
			// 验证远程端口是否匹配
			if pxyMsg.RemotePort != *matchedTunnel.RemotePort {
				ctl.xl.Errorf("隧道 '%s' 远程端口不匹配 (期望: %d, 实际: %d)",
					pxyMsg.ProxyName, *matchedTunnel.RemotePort, pxyMsg.RemotePort)
				return "", fmt.Errorf("[007] 远程端口不匹配")
			}
			// 验证本地端口是否匹配
			if pxyMsg.LocalPort != matchedTunnel.LocalPort {
				ctl.xl.Errorf("隧道 '%s' 本地端口不匹配 (期望: %d, 实际: %d)",
					pxyMsg.ProxyName, matchedTunnel.LocalPort, pxyMsg.LocalPort)
				return "", fmt.Errorf("[009] 本地端口不匹配")
			}

		case "http", "https":
			// 验证自定义域名
			if matchedTunnel.CustomDomain == nil {
				ctl.xl.Errorf("隧道 '%s' 缺少域名配置", pxyMsg.ProxyName)
				return "", fmt.Errorf("[005] 域名配置缺失")
			}
			// 验证域名是否匹配
			domainMatched := false
			for _, domain := range pxyMsg.CustomDomains {
				if domain == *matchedTunnel.CustomDomain {
					domainMatched = true
					break
				}
			}
			if !domainMatched {
				ctl.xl.Errorf("隧道 '%s' 域名不匹配 (期望: %s, 实际: %v)",
					pxyMsg.ProxyName, *matchedTunnel.CustomDomain, pxyMsg.CustomDomains)
				return "", fmt.Errorf("[008] 域名不匹配")
			}
			// 验证本地端口是否匹配
			if pxyMsg.LocalPort != matchedTunnel.LocalPort {
				ctl.xl.Errorf("隧道 '%s' 本地端口不匹配 (期望: %d, 实际: %d)",
		...
		case "xtcp", "stcp":
			// 验证密钥
			if matchedTunnel.SecretKey == nil {
				ctl.xl.Errorf("隧道 '%s' 缺少密钥配置", pxyMsg.ProxyName)
				return "", fmt.Errorf("[006] 密钥配置缺失")
			}
			// 验证客户端提供的密钥是否匹配
			if pxyMsg.Sk != *matchedTunnel.SecretKey {
				ctl.xl.Errorf("隧道 '%s' 密钥不匹配 (期望: %s, 实际: %s)",
					pxyMsg.ProxyName, *matchedTunnel.SecretKey, pxyMsg.Sk)
				return "", fmt.Errorf("[010] 密钥不匹配")
			}
			ctl.xl.Infof("隧道 '%s' 密钥验证通过", pxyMsg.ProxyName)
			// 验证本地端口是否匹配
			if pxyMsg.LocalPort != matchedTunnel.LocalPort {
				ctl.xl.Errorf("隧道 '%s' 本地端口不匹配 (期望: %d, 实际: %d)",
					pxyMsg.ProxyName, matchedTunnel.LocalPort, pxyMsg.LocalPort)
		...
		ctl.xl.Infof("隧道 '%s' 鉴权通过 (类型: %s, 服务端: %s)", pxyMsg.ProxyName, matchedTunnel.TunnelType, matchedTunnel.ServerName)
	}
