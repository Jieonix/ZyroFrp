...
func (svr *Service) RegisterControl(ctlConn net.Conn, loginMsg *msg.Login, internal bool) error {
	...
	xl.Infof("📡 客户端登录信息: IP [%s] 版本 [%s] 主机名 [%s] 系统 [%s] 架构 [%s]",
		ctlConn.RemoteAddr().String(), loginMsg.Version, loginMsg.Hostname, loginMsg.Os, loginMsg.Arch)
	xl.Infof("👤 用户信息: user=%s, runID=%s", loginMsg.User, loginMsg.RunID)

	// Check auth.
	xl.Infof("🔐 开始基础认证检查...")
	authVerifier := svr.authVerifier
	if internal && loginMsg.ClientSpec.AlwaysAuthPass {
		xl.Infof("🔓 内部连接，使用 AlwaysPass 验证器")
		authVerifier = auth.AlwaysPassVerifier
	}
	if err := authVerifier.VerifyLogin(loginMsg); err != nil {
		xl.Errorf("❌ 基础认证失败: %v", err)
		return err
	}
	xl.Infof("✅ 基础认证通过")

	// Check userKey in database
	if loginMsg.UserKey != "" {
		if exists, err := db.ValidateUserKey(loginMsg.UserKey); err != nil || !exists {
			xl.Errorf("用户 key '%s' 验证失败: %v", loginMsg.UserKey, err)
			return fmt.Errorf("userKey verification failed")
		} else {
			xl.Infof("用户 key '%s' 验证成功", loginMsg.UserKey)
		}
	} else {
		xl.Errorf("用户密钥为空，拒绝连接")
		return fmt.Errorf("userKey is empty, connection rejected")
	}
	...
	go func() {
		...
		ctl.WaitClosed()
		xl.Infof("🔚 控制器已关闭，正在清理...")
		svr.ctlManager.Del(loginMsg.RunID, ctl)
		xl.Infof("🧹 控制器清理完成")
	}()
	xl.Infof("🎉 客户端注册流程完成，连接已建立")
	return nil
}
